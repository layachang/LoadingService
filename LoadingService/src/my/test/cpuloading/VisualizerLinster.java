...
public class VisualizerLinster extends BasicFunc {
    ...
    public VisualizerLinster() {
        ...
        if(Loading.AUDIO_DEBUG) Log.v(Loading.TAG, "result: "+result);
    }
    ...
    protected void updateFftVisualizer(byte[] fft) {
        ...
        if(Loading.AUDIO_DEBUG) Log.v(Loading.TAG, "volumn= "+volumn);
        mAmount += volumn;
        mCount++;
        if (mCurrent < volumn)
            mCurrent = volumn;
    ...
    */
    ...
    private int getAvgVolumn() {
        if(Loading.AUDIO_DEBUG) Log.v(Loading.TAG, "mAmount= "+mAmount+"; mCount="+mCount);
        final int amount = mAmount;
        mAmount=0;
        final int count = mCount;
        mCount=0;
        if(Loading.AUDIO_DEBUG) Log.v(Loading.TAG, "amount= "+amount+"; count="+count);
